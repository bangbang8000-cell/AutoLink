...
def resolve_module_selection(conn, library: Optional[DeviceLibrary] = None) -> SelectionOutcome:
    ...
    V3.1.0-T1（本地定制）：``INVENTORY_MODE=True`` 时在第 1 步之后、第 2 步之前插入
    「清单化选型」分流（每口一模块 + MPO），与历史报价口径一致；该模式下不做降级。
    """
    ...
    # 2. 清单化选型模式（V3.1.0-T1 本地定制）——必须排在「双绞线豁免」之后（网线链路
    #    仍归 not_applicable，不得混计为缺口），且先于常规三态路径（不做速率降级/
    #    光纤类型放宽）。开启时每口一个与端口速率匹配的 SR 多模模块，与历史报价口径一致。
    if INVENTORY_MODE:
        if library is None:
            try:
                library = get_device_library()
        ...
        inv_sel = select_inventory_module(conn.a_module or '', distance, library)
        if inv_sel is None:
            # AL-F3 契约：status != matched 时**必须**给出可排期原因，不得只给计数
            return SelectionOutcome(
                selection=None,
                status='unmatched',
                reason=_unmatched_reason(conn.a_module or '', distance, False, library),
            )
        return SelectionOutcome(
            selection=inv_sel,
            status='matched',
            reason=getattr(inv_sel, 'match_reason', '') or '',
        )
