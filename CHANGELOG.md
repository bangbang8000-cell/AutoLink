...
> 按需求方 P1-11 §八-2 / §10.2 的明确约定**收紧**，并修掉其中两个开关的
> **空转 / 不可配**缺陷。
> 对外契约**纯增量**，`schema_version` 仍为 **2**：不删段、不改段结构。
> ⚠️ **golden 基线同批重生成**（27/28 文件）：全部差异已逐条归因到
> 「接入上联口 8 → 6」（上联需求与汇聚框数按 3/4 缩放），**无未解释差异**；
> 逐模板归因见 `docs/AL_5.3.0逐模板端口守恒对照表_2026-09-19.md` 的 5.3.1 节。
> **MC 不同批**：MC 无业务网口径领域，按双仓各自独立演进的决策，本版本 **AL 独立发布**。

### 数值口径变化（⚠️ 下游数字会变）

- **接入上联口默认 8 → 6**（`biz_access_uplinks`）
  MLAG 对中 2 口作 peer-link，北向**实际**上联仅 6 口（需求方口径）；
  与验收口径自洽：64 接入 × 6 = 384 = 2 台汇聚 × 192 口（8 槽 6 板）。
  ⇒ 全部含业务网的模板，**上联需求与汇聚框数按 3/4 缩放**：
  超大-2048 需求 1504 → **1128**、框数 47 → **36**；
  中型-512 需求 384 → **288**、框数 12 → **9**。
  **接入台数不变**（分组粒度默认仍 `merge`，未动）。
  **存量 JSON 项目结果会变**（该键此前在 JSON 路径不可配、恒取默认值）；
  **显式配置过该键的 INI 项目不受影响**。

### 缺陷修复

- **P1 · 分组粒度 `classify` 是「空开关」**（`backend/designer.py` / `backend/topology.py`）
  5.3.0 把它实现成「向下取整 + 余数再加一组」，与默认 `merge` 的向上取整
  **数学恒等** ⇒ 开关扫出 **0 差异**、从未生效，且**不报任何错**。
  修复：按需求方 §10.2 原文语义重写为「**各服务器类别分别取整后求和**」。
  验证：GPU 1250 / 存储 70 / 通算 60 @ 每组 45 台 ⇒ 默认 **31** 组，分类 **32** 组。
  同时补齐跨模块同口径：组数口径（`_num_access_groups` / `_biz_num_access_groups`）
  与实际建链分组（`_server_group_indices`）**共用同一份类别台数**，
  杜绝「算出来的组数」与「真建出来的连接」再次脱钩。

- **P0 · INI 项目启用业务网即崩（抛错 `AttributeError`）**
  5.3.0 的三项口径开关（`biz_agg_oversubscription` / `biz_agg_chassis_spec` /
  `biz_group_granularity`）只在 JSON 初始化路径赋值，INI 路径下这三个属性**根本不存在**。
  5.3.0 因它们「只赋值、无人读取」而未暴露；本版把类别台数真正接进建链后立即炸出。
  修复：抽出**唯一**初始化入口 `_init_biz_caliber_switches()`，JSON 与 INI 两条路径共用。

- **P2 · `biz_access_uplinks` 在 JSON 项目里不可配**
  同族四个口径键中唯一不可配的一条：其余三个都能在 `project_config.json` 的
  `topology` 段配置，唯独上联口写什么都没人读。修复：JSON 路径同样读取该键，
  默认仍 **6**、非法值回退 6（**不改变现有结果**）。

### 测试

- 新增 `tests/backend/test_caliber_531.py`（**23** 例）：三项口径的**生产者→消费者端到端**
  守卫（按 AL-G11 教训 —— 手搓配置的单测会绕过生产者）。核心是**空开关回归守卫**：
  断言「分类粒度必须与默认粒度有可观测差异」，一旦回退到数学恒等实现立即变红。
- `test_port_conservation_530.py` 中 8 处硬编码数字按新口径重推（需求按 3/4 缩放、
  框数同步下降）；**不是放宽断言** —— 判据仍是「需求 == 实收」「丢弃 == 0」。
- `tests/backend/test_designer_integration.py::TestDesignerBiz` 此前**本地即红**
  （INI 路径 AttributeError），本版一并修复转绿。

## [5.3.0] - 2026-09-19

> **R1 建模层修复**：修复**业务网端口守恒缺陷** —— 汇聚交换机台数由「服务器数查表」改为
> 「上联端口需求推导」，根治**连接被静默丢弃**（全 23 套模板中 **11 套** 欠连，缺口最大
> **66.0%**，且 `valid` 仍报 `True`）。本版本**同时**补上缺失的 **V021** 端口守恒校验规则
> 与 `reportData.port_conservation` 台账，使该类缺陷**不再可能静默通过**。
> 对外契约**纯增量**（新增 `port_conservation` 段，不删段、不 bump `schema_version`）。
> ⚠️ **golden 基线同批重生成**（18/28 文件）：11 套欠连补齐（框数上升）、2 套虚高修正（框数下降）；
> 逐条归因见 `docs/AL_5.3.0逐模板端口守恒对照表_2026-09-19.md`。
> **MC 不同批**：MC 无 datacenter/room/rack 领域，按 2026-09-18 双仓各自独立演进的决策，本版本 **AL 独立发布**。

### 缺陷修复

- **P0 · 连接被静默丢弃（`topology.py` 六处 `except ValueError: continue`）**
  端口耗尽时只 `print` 一行警告即 `continue`，连接**实际未建立**但**计数照加**，
  且 `validation.valid` 仍为 `True`。修复：新增 `_record_drop()` 与 `dropped_link_count` 属性，
  六处静默点**全部登记**为结构化记录（`network_type` / `device` / `port` / `reason` / `count`），
  经 `biz_info.dropped_links` 透出到校验层与契约层。

- **P0 · 框数推导与端口需求脱钩（`designer._calc_biz_chassis_frames`）**
  原按 `biz_chassis_frames_map` 以 `total_servers` 查表得框数，
  与真实上联需求无关：超大-2048 需 **1504** 口却只给 **16** 框（**512** 口），
  缺口 **992** 条（**66.0%**）。修复：新增 `_biz_num_access_uplinks()` 按
  `ceil(服务器 / 每接入下联口) × 2（MLAG 双上联） × 上联口数` 推导需求，
  再 `ceil(需求 / 单框下行口)` 得框数；仅当项目**显式配置** `biz_chassis_frames_map`
  时保留查表（向后兼容）。

- **P0 · 自检对「汇聚」错误豁免 2:1 收敛（`designer.validate_topology`）**
  原逻辑把 `'汇聚' in sw.name` 与 Spine 同等豁免 `max_ports * 2` ⇒
  业务网 1:1 结构的欠连**永远判不出来**。修复：仅 **Spine** 保留 2:1；
  汇聚按可配收敛比（默认 1.0 严格守恒）。

- **P1 · 缺失的 V021 校验规则（`validation._rule_port_conservation`）**
  文档早已列出 V021「端口守恒」，**代码中从未实现**（规则数 21 而非 22）。
  修复：按 `port_conservation.layers` 逐层比对「上联需求总数 vs 汇聚下行总口 × 收敛比」，
  不守恒或有丢弃 ⇒ **ERROR**；配置缺失 ⇒ **WARNING**（绝不静默通过）。

### 新增能力（用户反馈 P1-11「口径开关」）

- **`biz_agg_oversubscription`**（浮点，默认 **1.0** = 严格守恒；≥1.0，非法值回退）
  允许显式放开汇聚收敛比；开启后 `收敛比` 字段在报告中**显式标注**。
- **`biz_agg_chassis_spec`**（整数，默认 **32**；合法值 32/64/128，非法回退 32）
  单框下行口规格可配。
- **`biz_group_granularity`**（默认 `merge` = 现状 ceil 合并；可设 `classify`）
  接入分组粒度口径开关。
- 三项开关的**当前取值**经 `port_conservation.口径`（`data` 侧为 `calibers`）暴露，
  下游可据实判断数字按哪套口径算出。**默认值一律保持现状**。

### 新增契约

- **`reportData.port_conservation`**（`_REPORT_SECTIONS` 11 → **12** 段）
  逐层（`biz` / `oob`）台账：`服务器数` / `接入台数` / `接入上联口每台` / `上联需求总数` /
  `汇聚框台数` / `单框下行口` / `汇聚下行总口` / `收敛比` / `是否守恒` / `余量百分比` /
  `丢弃链路数`；汇总层给出 `检查层数` / `守恒层数` / `不守恒层数` / `总丢弃链路数`。
  `data` 侧键名为英文 snake_case（经 `_REPORT_KEY_MAP`）；`legacy_data` 保留中文键。
  **`schema_version` 保持 `2`**（纯增量）。

### 工程改进（防复发门禁）

- **`scripts/check_changelog_claims.py`**（新脚本，CI 新增步骤）
  CHANGELOG 中以反引号包裹的代码标识符须在源码中**可检索到**，
  否则报告「无代码支撑」—— 防止「宣称交付了、实际 0 命中」（5.2.2 的 `522-M3` 教训）。
  默认只校验**最新版本段**，历史段漂移不阻塞 CI。
- **CI 新增「合并冲突残留标记」扫描**（`git grep` 匹配 `<<<<<<<` / `=======` / `>>>>>>>`）。
- **golden 基线同批重生成**：18 个文件（13 模板 + 3 双平面 + 2 ZCube），
  变更前后均 `gen_golden.py --check` 全绿。

### 已知限制（不在本版本范围）

- **AL `cost` 仍为量级估算**（与真实价量级差 3–11 倍）：本版**只动"数量"**，
  单价与金额归**下游报价软件**。
- **机柜/功率口径**：新增汇聚交换机会计入机柜与功率（13/23 套模板受影响）。
  「业务汇聚是否应与 GPU 服务器混装」属**工程口径问题**，需产品明确后另起需求。

### ⚠️ 数值口径变化公告（消费方必读）

> 本版本**修正了一处系统性少配**，因此交付物的**交换机数量、机柜数、总功率**会发生变化。
> 下游（报价 / 交付 / 采购）**必须**据本节复核存量与在谈项目的数字，**不要**直接沿用旧值。

| 影响面 | 变化幅度 | 说明 |
|---|---|---|
| **业务汇聚交换机台数** | **↑ 11 套**（最大 +31 台）、**↓ 2 套**（各 −1 台） | 原数量**系统性偏低**；超大-2048 由 16 → **47** 框 |
| **连接总数** | 相应上升 | 补齐的是**原本被静默丢弃**的连接（如超大-2048 补 **+992** 条） |
| **机柜数** | **13/23 套变化**；仅超大-2048 净增 **1**（2338 → 2339） | 新增汇聚交换机计入机柜 |
| **总功率** | **13/23 套上升**，区间约 **+0.6 ~ +6.2 kW** | 新增交换机功耗；H100-100台 / 空项目 **下降 200 W**（框数 4→3） |
| **`valid` 语义** | **收紧** | 此前欠连仍报 `True`；现在欠连即 **ERROR** 且 `valid=False` |

**逐模板明细**见 `docs/AL_5.3.0逐模板端口守恒对照表_2026-09-19.md`（含每套的需求 / 框数 / 缺口 / 机柜 / 功率）。

**消费方迁移指引**：

1. **新项目**：无需动作，直接使用新数字。
2. **存量已交付项目**：若已按旧数字采购/报价，需**重新评估差额**（业务汇聚交换机 + 相应机柜/功率）。
3. **契约读取**：`reportData.port_conservation` 段给出逐层台账与**口径**（`calibers`），
   可据此判断数字按哪套口径算出，不必反查项目配置。
4. **兼容性**：`legacy_data` 保留中文键；`schema_version` 仍为 `2`；旧消费方**不读新段也不会报错**。

## [5.2.5] - 2026-09-18

> **R0 止血补丁**：修复**光模块选型系统性误配**——把与链路速率不符的光模块装配上去，
> 从而**污染每一个项目的模块总数与成本估算**。本版本**只修正确性**，无新功能；
> 对外契约**纯增量**（不删段、不 bump `schema_version`）。
> **MC 不同批**：MC 无 datacenter/room/rack 领域，按 2026-09-18 双仓各自独立演进的决策，本版本 **AL 独立发布**。

### 缺陷修复

- **P0 · 跨速率误配（两个独立根因，都会污染计数）**

  - **根因一：速率解析基准错误（`_parse_speed`）**
    旧实现用 `re.search(r'(\d+)')` 只取**首个数字段**，因此 **`'1.6T'` 被解析为 `1`**（而不是 1600）。
    后果：设备库中 **8 个 1.6T 光模块在速率维度上等价于 1G 模块**，于是在**严格匹配路径**（并非降级路径）
    被 **1G 带外网线链路**选中。同一仓库另有三份实现（`exporter._parse_speed_gbps` / `engine` / `validation`）
    单位口径正确（T/TB = ×1000），本处是**唯一的异类**。
    修复：改为**单位感知**解析（`T`/`TB` = ×1000），与其余三份口径一致。
  - **根因二：降级分支忽略速率（AL-F2）**
    候选为空时按"**距离最近**"降级，**不校验速率**，于是 25G 业务链路被装配 100G / 400G 模块。
    修复：降级被**限定在同一速率档内**——只允许**放宽光纤类型**（T2），**绝不跨速率**；
    同速率内确无档位时**返回「未匹配」并给出原因**，不再静默装配别的速率。

- **P0 · 铜缆/双绞线链路不再参与光模块选型（AL-F1）**
  `网线` / `双绞线` / `UTP` / `STP` / `CAT5..CAT7` 归类为 `not_applicable`（无需光模块），
  **不计入 `modules` 与 `cost`**。注意：**`DAC` / 铜缆仍是有效光模块形态，不在豁免之列**
  （`om_*_dac_*` 系列需被正常选中）。

- **P0 · 潜伏隐患：`library` 被误传到 `fiber_type`（实施阶段查出）**
  `select_module_for_connection` 以**位置参数**传 `library`，恰好落到第 4 位形参 `fiber_type` 上，
  使「按库匹配」静默失效、退化为默认库。修复为**关键字传参**并加 `match_reason` 兜底。

- **P1 · `cost` 增加口径标注（AL-F4）**
  本版本 `cost` 仍为**启发式估算**（与真实价量级差 3–11 倍），因此新增
  `可用于报价: False` / `口径` / `价格来源` 等标注键，**既有 4 个键名不变**。
  > 面向"可直接报价"的真实价量接入**不在本版本**（列入 5.3.0）。

- **P1 · 更正 `[5.2.2]` 的四项过度宣称（AL-F7）**
  522-M3 宣称的 `doctor` 依赖自检 / `pod_sizing` / `param_counts` / 重依赖下沉**在代码中不存在**
  （全仓检索命中 **0**）。已在本文件 `[5.2.2]` 段**新增**更正块逐条给出实际状态，
  **历史原文保留不删**。此四项**无对外契约影响**，属"文档超前于实现"。

### 契约增量（**不破坏**）

- `reportData` **新增 `module_selection` 段**：三分类计数 `matched` / `not_applicable` / `unmatched`
  \+ 未匹配明细聚合（网络类型 / 速率 / 线缆类型 / 原因 / 条数 / 样例连接），明细超限截断并给出 `截断数`。
- `schema_version` **保持 2**（纯增量，不 bump）；**原有 10 段全部保留**（`overview` / `architecture` /
...
> 本版本会**改变既有项目的模块计数与成本估算**，方向是"**更正确**"：
> **跨速率误配被清零**，同时新增两类**显式**分类（无需光模块 / 未匹配），
> 因此"总条数"会**下降**——这不是功能回退，而是**把此前静默错配的条数挤了出去**。

典型前后对照（同一模板、同一配置，仅换选型器）：

| 模板 | 唯一连接 | 旧：匹配条数 | 新：匹配条数 | 旧：1.6T 误配 | 新：1.6T 误配 | 新：无需光模块 | 新：未匹配 |
|---|---|---|---|---|---|---|---|
| `hygon_dcu_cluster` | 2352 | 2352 | 1856 | **160** | **0** | 160 | 336 |
| `H100-128台-IB` | 6260 | 4212 | 3212 | **324** | **0** | 324 | 2724 |
| `H100-256台-RoCE` | 24468 | 24468 | 22472 | **648** | **0** | 648 | 1348 |
| `DP3Tier-1024` | 127624 | 94856 | 121216 | **2080** | **0** | 2080 | 4328 |

- **hygon_dcu_cluster**：1.6T 误配 **160 → 0**（这 160 条是 1G 带外**网线**链路，现归入"无需光模块"）。
- **新出现的"未匹配"是设计如此**：设备库**确实没有** 25G / 10G 光纤档位，此前是靠"跨速率降级"用
  100G/400G 模块**顶替**，现在**如实报缺**。示例（hygon）：
  业务网 25G 光纤 ×320、带外网 10G 光纤 ×16 —— 原因均为「库内无 25G/10G 档位（同速率降级后仍无候选）」。
- **未放宽任何断言**：`test_optical_selector.py` 中 `test_no_speed_match_fallback` 改为
  `test_no_speed_match_returns_none`（断言 `is None`）——该用例**原本断言的正是缺陷行为**，
  属**修正断言**，已在提交信息与测试计划中明写，非静默改绿。

### 全量验证（发布准出证据）

- **23 套模板全量扫描**：**1,898,224** 条唯一连接 —— **守恒 0 例外**（匹配 + 无需 + 未匹配 == 总数）、
  **速率不符 0 条**、**异常模板 0 套**。
...
- 设备库 **127** 款（92 硬件 + 35 光模块）、场景模板 **23** 套、校验规则 **21** 条（V021 缺号）。
- 单柜功率上限默认 **12000W**。
...
- **连接去重键「方向不敏感」，误删反向连接（E005 / E011）**
  - 后端遍历 `dev.connections` 时已有 `if conn.a_device != dev.name: continue` 过滤，保证**每条连接对象只输出一次**；但 7 处去重键写作 `tuple(sorted([a_device, z_device])) + (a_port,)`——端点一排序，**方向信息就丢了**。只有当**两端端口号巧合相等**时才会把反向连接误判为重复而删除：
    `OOB汇聚_1` 用 端口51/52 下行到 `OOB接入_26`，而 `OOB接入_26` 自身上行也恰是 端口51/52 → 这 2 条反向连接被丢弃。
  - 症状：`E005: 连接表行数 46904 与设计连接数 46902 不一致`——**导出侧（连接表，不去重）是正确的，设计侧少算 2 条**；`E011`（布线指导表 vs 设计连接数）因两侧同源出错而彼此吻合，长期掩盖了该缺陷。
  - 修复：7 处统一改为方向敏感键 `(a_device, z_device, a_port)`。这不是新增约定——`zcube_topology.py` 早已如此，其注释直书「双向边各输出一次，**与 design 一致**」，本缺陷属偏离既有约定。
  - **基线影响**：28 个 golden 模板中 **9 个**命中该碰撞，已同批重生成（以 OOB 为主；`dual_plane_1024_b300_800g` / `zcube_1024_quad_nic` / `zcube_512_dual_nic` 另牵涉参数网与存储网）。连接计数**只增不减**，符合「此前为系统性欠计」的判断。

- **CI 红灯修复（523 系列遗留）**
  - `mcp` 依赖加 `<2` 上限（上游 2.x 为破坏性变更）。
  - 挂死用例治理：曾把作业拖到 48 分钟仍无输出、只能人工取消的用例，补**作业级 `timeout-minutes: 40`** 与 `pytest --timeout` **双保险**。
  - 旧契约断言对齐当前实现：`fixit` / `knowledge` / `manage` / `migration` / `optimization` 等（含 INI 机柜功率默认 12000W、以及「收敛比建议」在自定义模式下行口被钳制到 `switch_ports // 2` 后的可达性）。

### 文档与文案修正

- **「每柜 GPU 服务器台数」措辞明确化**：创建向导标签与机房设计导出的「每柜 GPU 数量」「每柜GPU数」易被误读为 **GPU 卡数**（8 卡节点下可能被按卡折算机柜，使机柜数**虚高 8 倍**）。已统一改为「**每柜 GPU 服务器台数**」，并在界面提示中写明「计**台数**、不计卡数，默认 1 台/柜」。

- **用户手册机柜章节更正**
  - 单柜功率上限默认值由 **6000W 更正为 12000W**（与 `config_schema.py` / `designer.py` / 前端 `rack.store` 的 12000 一致；6000 仅存于 `project_config.create_default_config` / `migration` / `room` / `fixit` 等旧路径，属遗留不一致，待后续统一）。
  - 补充散热方式联动上限：风冷 ≤ 15KW/柜、冷板液冷 ≤ 60KW/柜、浸没式 ≤ 100KW/柜。
  - 新增「一柜多台」的正确操作：**必须同时调整两处** —— 功率上限提到目标值（如 `36000` = 36KW 支持一柜 2 台）**且**每柜台数上限改为 `2`。只调功率不够：台数上限仍为 1 会拦住第二台（例：H100 约 10.2KW，在 36KW 柜中仅占 28%、不触发自动独占，但仍被默认台数上限挡下）。

- **`gpu_dedicated` 默认值表述精确化**：「GPU 一柜一台」的**结论成立**，但归因需精确：
  - 代码**兜底**值（未显式配置时）仍是 **`false`**（`designer.py` / `rack_allocation.py`）；只有 INI 旧格式路径默认 `true`。
  - 真正保证「一柜一台」的是三件事叠加：**模板层全部显式开启**（23 套模板的 `project_config.json` 均写入 `gpu_dedicated: true`）、**功率 ≥ 单柜上限 50% 自动独占**、**`gpu_per_cabinet` 默认 1**。
  - 因此「默认改为 True」的说法应理解为「**模板层默认开启**」，而非「代码兜底改为 True」。

### 内容真值（以代码为唯一真值源）

设备库 **127 款**（92 硬件 + 35 光模块）、场景模板 **23 套**、校验规则 **21 条**（V021 缺号）。文档中的这些数字由 `scripts/check_doc_numbers.py` 在 CI 中反向校验。

## [5.2.2] - 2026-09-17

> **更正（2026-09-18）**：本节「里程碑」中的 **522-M3** 条目存在**四项过度宣称**——所列能力在代码中**并不存在**，不应计为已交付。以下逐条给出**实际状态**（证据为全仓检索 `backend/` + `docs/`，2026-09-18 复核）。**本节历史条目原文保留不删**，以留可追溯记录；后续版本按实际状态重新规划。
>
> | 曾宣称（522-M3） | 实际状态（2026-09-18 复核） | 证据 |
> |---|---|---|
> | `doctor` 依赖自检 | **未实现**，全仓检索命中 **0**；CLI 也无 `doctor` 动作 | `grep -rn doctor backend/` → 0 |
> | `pod_sizing` 参数化 | **未实现**，全仓检索命中 **0** | `grep -rn pod_sizing backend/` → 0 |
> | `param_counts` 参数化 | **未实现**，全仓检索命中 **0** | `grep -rn param_counts backend/` → 0 |
> | 重依赖下沉 | **未实现**。AL 侧**无** MC 那样的 `_deps.py` 惰性导入层；检索无 `_deps.py` / `lazy_import` | `grep -rn "_deps.py\|lazy_import" backend/` → 0 |
>
> **确实已交付**（同段其余项，经复核成立）：`selfcheck` 真实化（`manager.py:798` 有实现，且校验屏蔽规则对账状态）、
> 修复 `cli.main()` 全局劫持 `builtins.print` 的进程级副作用、冷启动预热的部分改动。
> **口径纠正**：522-M3 的准确表述应为「**进程卫生与 `selfcheck` 真实化**」，性能类（重依赖下沉 / 依赖自检 / 参数化）**未交付**，已在 5.2.5 的 PRD 中作为 `AL-F7` 立项更正。
>
> 影响评估：上述四项**均无对外契约影响**（无 API/MCP 工具/CLI 语义变化），属「文档超前于实现」而非「功能回退」；
> 但会误导下游对**冷启动/依赖治理**能力的预期，故必须更正。

### ⚠️ 破坏性变更（升级前必读）

**CLI 退出码语义变更（AL-E1 / `DP-AL-02` 定稿，无兼容开关）**

旧行为：业务失败（如配置文件不存在、模板不存在、结果为空）**也返回退出码 0**。
新行为：

| 退出码 | 含义 |
|---|---|
| 0 | 成功 |
| 1 | 内部异常（handler 抛出未捕获异常） |
| 2 | 参数或配置错误（`AL_ERR_CONFIG` / `AL_ERR_INVALID_ARGS`） |
| 3 | 执行失败（`AL_ERR_EXEC` / `AL_ERR_EMPTY_RESULT`） |

- **空结果即失败**：`{}` / `[]` / `null` 一律按 3 处理，杜绝"无声失败"。
- stdout **始终**输出结构化 JSON（含 `error_code`），失败原因**同时**写 stderr。
- **迁移指引**：下游脚本若以 `$? == 0` 判定成功，现在可以正确使用；若此前依赖"恒 0"，
  需改为解析 `success` 字段或按新退出码分支处理。详见 `docs/cli.md` §9。

**其他行为变更**
- `export` 缺省 `--output-types` 由「空（静默 no-op）」改为「**全部可用类型**」（AL-E5）。
- `export` 新增 `--no-archive`（无副作用取值）与 `--regenerate`（强制重算）开关（AL-E4）。
- MCP `tools/call` 失败时正确置 `isError=true`，响应扁平化为单层并携带 `error_code`。

### 里程碑

> 为避免与 5.2.0 的「里程碑 M1–M5」编号冲突，5.2.2 的里程碑统一记为 **`522-M1` ~ `522-M4`**。

- **522-M1 对外契约止血**：MCP 工具名/参数泄漏修复、屏蔽表语义规则化 + 启动断言、
  `inputSchema` 接入、错误语义统一（`isError` + 扁平化 + `error_code`）、
  设备检索含 `id` + 新增 `device get --id`、CLI 退出码契约。
- **522-M2 可信门禁与进程卫生**：权限 `annotations`、`confirm` 门禁（与 `write_gate` 同层叠加）、
  stdio 开关约束、`selfcheck` 真实化、`prompts`/`resources` 注册、
  **修复 `cli.main()` 全局劫持 `builtins.print` 且不还原的进程级副作用**（曾导致同进程后续调用输出被污染）。
- **522-M3 引擎正确性与性能**：重依赖下沉、`doctor` 依赖自检、冷启动预热、
  `pod_sizing` / `param_counts` 参数化、进程级副作用与并发安全。
- **522-M4 文档与门禁**：`docs/cli.md` 退出码表与导出章节对齐、README 口径校正
  （设备库 **127** = 92 硬件 + 35 光模块；校验规则 **21** 条，V021 缺号；场景模板 **23** 套）、
  双端同构结构比对。

### 文档体系整理

- **版本单源校正**：`version.json` 由 `5.2.0` 补正为 `5.2.2`（此前 CHANGELOG / README 已到 5.2.2 而单源未同步），
  经 `scripts/sync_version.py --set 5.2.2` 同步 `package.json` / `package-lock.json` / `VERSION`。
- **新增文档索引** [`docs/README.md`](docs/README.md)：按角色导航 + 文档清单 + 「文档 ↔ 代码真值」对照 +
  防漂移维护规约（改代码时该同步哪些文档、哪些禁止入库）。
- **`docs/deployment.md` 全面重写**：原文档停留在 v3.6.0（模板 19 / 设备库 126 / 规则 22 条等均已过期）。
  新区分 11 节，补齐 **Agent Connect 部署**章节（启动参数 / 前置条件 / 5.2.2 可信门禁 / 自检）、
  AI Hub 端口与鉴权、**导出归档与指纹复用**策略、CI 双工作流（`ci.yml` push main / `build.yml` tag）说明。
- **`docs/user_guide/user_guide.md` 更新**：版本对齐 5.2.2；修正设备库规模（118+34 → **127** = 92 + 35）、
  本地能力矩阵（模板 19→**23**、设备库 126→**127**、校验 V001~V020 → **21 条含 V022**）；
  新增「归档、复用与重算」「机器可读契约（`schema_version` / 英文规范子树）」「CLI 退出码契约」三节，
  以及第 24 章「5.1 / 5.2 系列亮点」；修正 `18.5` 章节编号错位。
- **README 校正**：Agent 工具数 60 → **72**；测试规模改为实测值
  （Vitest **99** 文件 / **1276** 用例；pytest **86** 文件 / **1613** 用例）；安装包产物名对齐 5.2.2；
  路线图 5.1 「规划中」→ **已完成** 并补 5.2 系列；文档章节改为表格并补齐各文档入口。

### 文档结论修正

- 此前反馈中的「`export` 重跑完整设计（拓扑自检 ×2）」经**堆栈埋点复核为误判**：
  命中的 2 行是同一次自检的标题行与结果行，`handle_export` 全流程仅 1 次设计构造 + 1 次自检。
  该条已收窄为「产物复用 + 无副作用模式」。
...
- **Agent Connect（MCP Server，5.1.1–5.1.10 全量）**：以标准 MCP 协议接入外部 AI Agent / 编程智能体（Claude / Codex / Trae Work / VS Code 等），直接查询/创建/更新/渲染项目、模板、设备库、机房规划、输出。
  - **双场景模型**：编译态（产品使用，不修改软件本体）/ 源码态（开发者，CLI 透传 + 沙箱文件系统）；
  - **确定性语义层**：入参契约（L1 JSON Schema）/ 语义校验（L2）/ 幂等事务（L3）；
  - **异步任务**：task_id + 进度轮询 / 等待 / 取消；**操作审计**（脱敏）；**远程模式试点**（TLS + 强鉴权 + 按域授权）；**Agent 反馈自优化**闭环。
- **MCP 接入指南（5.1 收尾）**：帮助菜单新增「MCP 接入指南」（工作区加载 + 一键复制接入配置）；设置 → AI → Agent Connect 提供「复制接入配置 / 打开指南」按钮；用户指南同步更新。
- **4 项问题修复（5.0.11 内容并入）**：
  - **工作台多语言**：切换语言即时跟随（AIDC 规划子视图全量 i18n + 补齐 workbench/rack 缺失 key）；
  - **日志面板亮色适配**：黑底 → 主题 token；
  - **AIDC 规划 GPU 规模联动**：随项目动态回填（普通项目回退读 project_config.json）；
  - **PDF 设计报告**：移除静态目录页；下行口钳制到 switch_ports//2（128 台 GPU：64 口交换机 Leaf 32 + Spine 16、128 口交换机 Leaf 16 + Spine 8）；GPU 一柜一台（gpu_dedicated 默认 true）。
- **工程修复**：ci.yml 移除无效步骤属性（workflow 可运行）；build:electron 类型断言修复；golden 基线更新。

## [5.0.11] - 2026-09-07

### 5.0.11 修复发布（4 项问题修复 + 改进）

- **工作台多语言修复（511-a）**：AIDC 规划子视图全量接入 i18n（新增 `aidc` 命名空间，5 语言全覆盖）；补齐工作台/机柜缺失翻译 key（组网设计/机柜设计/可渲染就绪、机房/机柜设计 Excel 导出、定稿/归档/清空等）；渲染结果类型标签与「打开文件/打开文件夹」提示接入 i18n（时间戳跟随界面语言）。**切换语言后工作台视图即时跟随**。
- **日志面板亮色适配（511-b）**：LogPanel 日志区与 DiagnosticsPanel 错误日志区由硬编码黑底（`bg-gray-900`）改为主题 token（亮色浅灰 / 暗色 app-surface），亮色主题下不再黑底，分级色同步亮暗适配。
- **AIDC 规划 GPU 规模联动（511-c）**：独立 AIDC 规划 Tab 透传当前项目（`WorkspaceView → AidcPlannerTab → AidcPlannerPanel`）；普通 AL 项目（无 plan.json）load 失败时回退读取 `project_config.json` 的 `num_gpu_servers` 回填 GPU 规模，不再固定停在默认 64。
- **PDF 设计报告修复（511-d）**：
  - **移除目录页**：删除无页码/无跳转的静态假目录，封面后直接进入章节；
  - **下行口数钳制**：custom 模式 `downlink_limit` 钳制到 `switch_ports//2`（至少留一半上联口），杜绝满口下联、无上联口的错误 Leaf 结构——128 台 GPU：64 口交换机 **Leaf 32 + Spine 16**、128 口交换机 **Leaf 16 + Spine 8**；
  - **机柜一柜一台 GPU**：`gpu_dedicated` 默认改为 True（GPU 训练服务器默认独占机柜，128 台 GPU → 128+ 柜），需要多台共柜的项目可显式设置 `false`。
- **golden 基线更新**：重新生成 32 个模板基线（补全新增 H100-256台-RoCE / H100-512台-RoCE，修正国产-昇腾-256 存储网结构），`gen_golden.py --check` 通过。

## [5.0.10] - 2026-09-06

### 5.0.10 内容资产升级与收官（5.0 系列第 10 版）

- **示例库扩充（510-a）**：示例库 4 → **7**，新增超大规模示例 **H100-256台-RoCE / H100-512台-RoCE**（覆盖 256/512 台规模要求）与**国产-昇腾-256 多厂商场景**（昇腾 910C 国产智算）；经 `gen_samples.py` 全量校验 7/7 通过。
- **内容库升级（510-b）**：场景模板 23 → **25**、设备库 **126** 台全量对账校验通过（索引/目录一致、id 唯一、字段完整、可互灌）。
- **AL 全量模板重测（510-c）**：23→25 模板逐模板重测全绿，未发现失效模板需下线；产出具结构化重测报告。
- 5.0 系列收官（510-e）：对照 5.0 PRD 十版目标全部达成，验收复盘归档。

## [5.0.9] - 2026-09-06

### 5.0.9 交付与运维强化（5.0 系列第 9 版）

- **升级体验（509-a）**：安装包下载**断点续传**（`.part` 记录偏移，失败续传）+ **SHA-512 强校验**（读 latest.yml sha512，缺失退化 Content-Length 比对）+ **版本回滚**（安装前留存上一版本安装包，启动异常可回退）+ **灰度通道**（stable/beta，按通道选择 latest\*-channel.yml，服务端多通道下发）。
...
- 平台侧 509-c：health/full 磁盘水位/进程探测 + 多通道 versions.json（sha512/镜像）+ 备份统一 auto_backup.sh + 告警脚本。
...
- **拓扑渲染优化**：TopologyTab 在视口内仅渲染可见节点/边（onlyRenderVisibleElements），大拓扑大幅降低 DOM 节点与帧耗时；大拓扑（≥1000 节点）下禁用 MiniMap 平移/缩放，减少缩放计算开销
- **性能基准对比基线**：scripts/bench_perf.py 新增 --json/--compare（规范化报告 + 与基线逐场景 delta/PASS/REGRESS）与 512 GPU 设计规模档（量化规模化设计成本）

### 基准（5.0.7 本地实测，阈值≤实测 PASS）

- 2048 GPU 设计/渲染：≤30s / 实测 0.47s
- 512 GPU 设计/渲染：≤8s / 实测 0.12s
- 225 柜机房落位：≤5s / 实测 0.08s
