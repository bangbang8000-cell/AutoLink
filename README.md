...
| **硬核校验** | **23 条**校验规则（V001–V023；5.3.0 补齐 V021 端口守恒、5.4.2 新增 V023 参数网 Leaf 上限）：拓扑连通性、端口容量、光模块匹配、功率上限、三合一融合域，杜绝"设计失守" |
...
- **23 条校验规则（V001–V023）**：拓扑连通性、端口容量、光模块匹配（含分裂线缆）、功率上限、ZCube/超节点/三合一融合域专项校验、参数网 Leaf 台数上限
...
![校验结果面板 - 23 条规则严重度分组与定位](snapshot/校验报告.png)
...
- **回归**：30 套模板闸门全过；CLI export 验证 5090 模板光模块 = 全 SR4+10G（无 1.6T/DAC/AOC），未开启清单化的模板（H100-128台-IB/RoCE 等）行为不变。
...
- **回归**：H200-256(64+32)、H200-128(32+16)、H100-512(三层128+128+64，4096卡超二层合理)、H100-128(32+16)、5090-500(32+16 不变)、5090-64(4+2) 全部合理；30 套模板闸门全过
...
│   ├── validation.py       #   23 条校验规则引擎（V001–V023；5.3.0 补齐 V021，5.4.2 新增 V023）
...
> 文档中的数字（设备库 131 / 模板 30 / 校验规则 23）由 `scripts/check_doc_numbers.py` 以**代码为唯一真值源**反向校验，CI 门禁拦截漂移。
